...
void initialize() {
	...
	chassis.addFourBar(frontArm, FRONT_ARM_GEAR_RATIO, {0.05, 0, 0, 0});
	chassis.addFourBar(backClaw, BACK_CLAW_GEAR_RATIO, {1, 0, 0, 0});
...
void opcontrol() {
	...
	while (true) {
		...
		if (backClaw.isOverTemp()) {
			pros::lcd::print(3, "Back claw is overheating: %f°C", backClaw.getMotors().at(0).getTemperature());
			// master.rumble(".");
		}
		...
		if (master.get_digital(pros::E_CONTROLLER_DIGITAL_R1)) {
			if (armPos.get() < ARM_UPPER_LIMIT)
				chassis.fourBarList.at(0).moveVelocity(100);
			else
				chassis.fourBarList.at(0).moveVelocity(0);
		} else if (master.get_digital(pros::E_CONTROLLER_DIGITAL_R2)) {
			if (armPos.get() > ARM_LOWER_LIMIT)
				chassis.fourBarList.at(0).moveVelocity(-100);
			else
				chassis.fourBarList.at(0).moveVelocity(0);
		} else
		...
		if (master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_L2)) {
			if (backClawOpen)
				chassis.fourBarList.at(1).startMove(BACK_UPPER_LIMIT);
			else if (!backClawOpen)
				chassis.fourBarList.at(1).startMove(BACK_LOWER_LIMIT);
		} else if (master.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_DOWN)) {
			chassis.fourBarList.at(1).startMove(backClawPos.get() - BACK_CLIMB);
		}
		cout << backClawPos.get() << "\t" << chassis.fourBarList.at(1).target << "\n";
